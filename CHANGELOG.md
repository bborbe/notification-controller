...
- feat: add `TelegramThrottle`, a soft channel-level rate limit on the Telegram lane. A throttled notification arriving with no open window is delivered immediately and opens a 5-minute window; further ones inside it are buffered and flushed as **one** coalesced message at the window edge, so a burst of manager gates arrives as at most two messages instead of one per gate. `account-loss-limit` and `account-profit-limit` bypass the throttle entirely — never buffered and never advancing a window, so a trading alert can be neither delayed nor able to suppress a gate queued behind it. The limit is deliberately soft: a burst straddling the window edge yields one extra message, because the post-flush arrival is a fresh leading edge. Windows are keyed per (chat, bot) rather than per chat, so an escalation is never coalesced onto the bot that must keep alerting. The buffer is in memory, so a pod restart loses a batch that has not yet flushed.
